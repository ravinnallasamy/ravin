 ...
 * imports `dashboardCache` and depends only on the interface.
 *
...
const globalForCache = globalThis as unknown as {
  __githubDashboardCache?: CacheStore;
};

export const dashboardCache: CacheStore =
  globalForCache.__githubDashboardCache ??
  (globalForCache.__githubDashboardCache = new MemoryCacheStore());
...
export async function readThrough<T>(
  ...
  ttlSeconds: number,
  producer: () => Promise<T>,
  store: CacheStore = dashboardCache,
