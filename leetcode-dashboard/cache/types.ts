 ...
 * swapping the in-memory implementation for Redis later is a one-line change in
 * `./index.ts` with zero churn elsewhere. The interface is async on purpose —
 * Redis is async, so callers already `await` today.
 *
 * Cache entries are keyed by `username + queryType` (see `constants.CACHE_KEYS`)
 * so multiple usernames never collide.
 */
